...
class SourceManager
{
    ...
    {
        $json = (string)$this->scopeConfig->getValue(self::XML_PATH_SOURCES, ScopeInterface::SCOPE_STORE);
        if (trim($json) === '') {
            return [];
        }
        ...
        if (!is_array($decoded)) {
            $this->logger->error('Cannot decode source definitions JSON');
            return [];
        }

        return array_values(array_filter($decoded, static function (array $source): bool {
